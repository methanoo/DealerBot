...
\<div align="center"\>
\<img src="[https://i.imgur.com/kS9Gw3v.png](https://i.imgur.com/kS9Gw3v.png)" width="450" alt="Schema Circuitale"\>
...
Utilizza questi codici per assegnare un'identità univoca a ogni telecomando giocatore.

| ID Giocatore | Chiamata Funzione | Valore HEX (Raw) | Stato |
| :---: | :--- | :--- | :---: |
...
#define IR_PIN 1      // Output segnale (Pin Fisico 6) // (1)
#define BUTTON_PIN 2  // Input Pulsante (Pin Fisico 7) // (2)
...
void loop() {
  if (digitalRead(BUTTON_PIN) == LOW) {
    ...
    sendNEC(1, 0x0, 0xC, 0); // (3)
    
    delay(500); // Anti-rimbalzo
  }
}
```
...
3.  **Codice NEC**: Inserisci qui il comando per distinguere i giocatori.
````
...
!!! danger "Step Finale"
    Prima clicca su **Strumenti > Scrivi Bootloader**, poi carica lo sketch tramite **Sketch > Carica tramite programmatore**.
```
...
\!\!\! info "Differenza tra i file e Metodo di Caricamento"
\* **`ATtiny_Sender.ino`**: Il firmware da caricare su ogni telecomando giocatore.
\* **`IR_Analyzer.ino`**: Sketch di servizio per mappare o verificare i codici HEX.
\* **`Arduino ISP`**: Lo sketch ponte che trasforma l'Arduino Uno in un programmatore hardware per chip ATtiny.

-----

### 💡 Consigli per il montaggio

  * **Batteria**: Se usi una CR2032 (3V), assicurati che il LED IR sia ad alta efficienza.
  * **Resistenze**: Non dimenticare una resistenza da circa 47-100 Ohm sul LED IR per proteggere il pin dell'ATtiny.
